...
<template>
  <section
    ref="heroRef"
    :class="[
      'relative min-h-screen flex flex-col overflow-hidden parchment-bg grid-texture',
      darkMode ? 'bg-slate-950' : 'bg-parchment-50'
    ]"
  >
    <!-- Cursor Glow Effect -->
    <div
      class="pointer-events-none absolute inset-0 z-0 opacity-30 transition-opacity"
      :style="{
        background: `radial-gradient(600px circle at ${glowX}px ${glowY}px, ${darkMode ? 'rgba(99,102,241,0.15)' : 'rgba(99,102,241,0.08)'}, transparent 40%)`
      }"
    />
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div :class="darkMode ? 'opacity-20' : 'opacity-30'">
        <div :class="['absolute top-20 -left-20 w-96 h-96 rounded-full mix-blend-multiply filter blur-3xl animate-blob', darkMode ? 'bg-purple-600' : 'bg-purple-200']"></div>
        <div :class="['absolute top-40 -right-20 w-96 h-96 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000', darkMode ? 'bg-indigo-600' : 'bg-indigo-200']"></div>
        <div :class="['absolute -bottom-20 left-1/3 w-96 h-96 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000', darkMode ? 'bg-pink-600' : 'bg-pink-200']"></div>
      </div>
    </div>
    ...
    <main class="relative z-10 flex-1 flex flex-col justify-center items-center text-center px-6 pb-24">
      ...
      <div
        :class="[
          'inline-flex items-center gap-2 px-4 py-1.5 rounded-full border mb-10',
          darkMode ? 'bg-white/5 border-white/10 text-indigo-300' : 'bg-white/60 border-indigo-100 text-indigo-600'
        ]"
      ...
      <!-- Main Title - Staggered Characters -->
      ...
      >
        ...
        <button
          @click="scrollToNext"
          :class="[
            'px-8 py-4 rounded-full border font-mono text-base tracking-wide transition-all flex items-center justify-center gap-2',
            darkMode
              ? 'border-slate-700 text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'
              : 'border-slate-300 text-slate-500 hover:bg-white/50 hover:text-slate-700'
          ]"
        >
          EXPLORE
    ...
    <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
      <div :class="['w-6 h-10 rounded-full border-2 flex items-start justify-center pt-2', darkMode ? 'border-slate-600' : 'border-slate-300']">
        <div :class="['w-1.5 h-1.5 rounded-full animate-bounce', darkMode ? 'bg-slate-400' : 'bg-slate-400']"></div>
      </div>
    </div>
  </section>
</template>
